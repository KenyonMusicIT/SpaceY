...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                html.Div(dcc.Graph(id='success-pie-chart', style={'width': '50%', 'display': 'inline-block'})),
                                html.Br(),
                                html.P("Payload range (Kg):"),
                                # TASK 3: Add a slider to select payload range
                                dcc.RangeSlider(id='payload-slider', min=0, max=10000, step=1000, value=[min_payload, max_payload]),
                                # TASK 4: Add a scatter chart to show the correlation between payload and launch success
                                html.Div(dcc.Graph(id='success-payload-scatter-chart', style={'width': '50%', 'display': 'inline-block'})),
                                ])
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(success_counts, values='count', names='class', title='Total Success Launches for Site ' + entered_site)
        return fig
